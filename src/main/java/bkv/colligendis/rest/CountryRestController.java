...
public class CountryRestController {
    ...
    public ResponseEntity<List<Country>> getCountriesWithNameFilter(@RequestBody Filter filter) {
        List<Country> countries = N4JUtil.getInstance().numistaService.countryService.findByNameContainingIgnoreCase(filter.getNameFilter());
        return ResponseEntity.ok(countries);
    }
//
//    @PostMapping("/csi/filter/name")
//    @ResponseBody
//    public ResponseEntity<List<CSIListItem>> getCSIListWithNameFilter(@RequestBody Filter filter) {
//
//        List<Country> countries = N4JUtil.getInstance().numistaService.countryService.findByNameContainingIgnoreCase(filter.getNameFilter());
//        List<Subject> subjects = N4JUtil.getInstance().numistaService.subjectService.findByNameContainingIgnoreCase(filter.getNameFilter());
//        List<Issuer> issuers = N4JUtil.getInstance().numistaService.issuerService.findByNameContainingIgnoreCase(filter.getNameFilter());
//
//        Stream<CSIListItem> csiListItemStream = Stream.concat(Stream.concat(
//                countries.stream().map(country -> {
//                    CSIListItem csiListItem = new CSIListItem(country.getEid().toString(), country.getName(), CSIListItem.Label.COUNTRY);
...
//                    CSIListItem csiListItem = new CSIListItem(subject.getEid().toString(), subject.getName(), CSIListItem.Label.SUBJECT);
...
//                    CSIListItem csiListItem = new CSIListItem(issuer.getEid().toString(), issuer.getName(), CSIListItem.Label.ISSUER);
