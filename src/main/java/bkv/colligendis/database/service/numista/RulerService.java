...
public class RulerService extends AbstractService<Ruler, RulerRepository> {
    ...
    public UUID findUuidByNid(String nid) {
        return findUuidByPropertyStringValue(Ruler.LABEL, "nid", nid);
    }

    public void detachIssuer(UUID rulerUuid, UUID issuerUuid) {
        detachEntityFromAnotherEntityWithRelationshipType(rulerUuid, issuerUuid, Ruler.RULES_WHEN_BEEN);
    }
    ...
    public boolean setName(UUID rulerUuid, String name) {
        return setPropertyStringValue(rulerUuid, "name", name);
    }

    public void setRulerGroup(UUID rulerUuid, UUID rulerGroupUuid) {
        setSingleOutgoingRelationshipToNode(rulerUuid, rulerGroupUuid, Ruler.GROUP_BY, RulerGroup.LABEL);
    }

    public void addRuleFromYear(UUID rulerUuid, UUID yearUuid) {
        addSingleOutgoingRelationshipToNode(rulerUuid, yearUuid, Ruler.RULES_FROM);
    }

    public void addRuleTillYear(UUID rulerUuid, UUID yearUuid) {
        addSingleOutgoingRelationshipToNode(rulerUuid, yearUuid, Ruler.RULES_TILL);
    }

    public boolean setActual(UUID rulerUuid) {
        return setPropertyBooleanValue(rulerUuid, "isActual", true);
    }

    public void setIssuer(UUID rulerUuid, UUID issuerUuid) {
        setSingleOutgoingRelationshipToNode(rulerUuid, issuerUuid, Ruler.RULES_WHEN_BEEN, Issuer.LABEL);
    }
     ...
     * This field uses while parsing new rulers by
     * <a href="https://en.numista.com/catalogue/get_rulers.php?country=...">...</a>
     * Also, it can be uses at the start of the year to extend the ruling period
     *
     * @param uuid Ruler's uuid
     * @return tru - if actual, or false - if not actual. Also can return null, if
     *         the field .isActual is missing
     */
