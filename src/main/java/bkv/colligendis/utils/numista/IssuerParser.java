    ...
    public IssuerParser() {
        super((page, nType) -> {
            ...
            if (nType.getIssuer() != null && nType.getIssuer().getCode() != null
                    && nType.getIssuer().getCode().equals(code) && nType.getIssuer().getName().equals(name)) {
                // DebugUtil.showInfo(IssuerParser.class, "The Issuer for nType is equal to
                // Issuer on the page.");
            ...
            try {
                ...
                DebugUtil.showError(IssuerService.class, "Find more then 1 Issuer by name: " + e.getParams().get("name")
                        + "or code: " + e.getParams().get("code"));
            }
            ...
            if (issuer == null) { // create new Issuer with code and name
                ...
                DebugUtil.showInfo(IssuerParser.class,
                        "Create new Issuer with (code, name) = (" + code + ", " + name + ")");
            }

            if (issuer.getCode() == null || issuer.getCode().isEmpty() || !issuer.getCode().equals(code)) {
                DebugUtil.showWarning(IssuerParser.class,
                        "For Issuer: " + name + " change a code from (" + issuer.getCode() + ") to (" + code + ")");
            ...
            if (!issuer.getName().equals(name)) {
                DebugUtil.showWarning(IssuerParser.class,
                        "For Issuer: " + code + " change a name from (" + issuer.getName() + ") to (" + name + ")");
            ...
            DebugUtil.showInfo(IssuerParser.class,
                    "Set an Issuer with (code, name) = (" + issuer.getCode() + ", " + issuer.getName() + ")");
