...
public class CurrencyParser extends NumistaPartParser {
    ...
    public CurrencyParser() {
        super((page, nType) -> {
            ...
            if(currency == null){
                parseCurrenciesByIssuerCodeFromPHPRequest(nType.getIssuer(), currencyNid);
            }
    ...
    private static boolean parseCurrenciesByIssuerCodeFromPHPRequest(Issuer issuer, String prefill) {
        final String issuerCode = issuer.getCode();

        Document currenciesPHPDocument = loadPageByURL(CURRENCIES_BY_ISSUER_PREFIX + "country=" + issuerCode + "&prefill=" + prefill);
