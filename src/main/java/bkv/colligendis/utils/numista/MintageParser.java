    ...
    public MintageParser() {
        super((page, nType) -> {
            ...
            if (anneesElement == null) {
                DebugUtil.showError(MintageParser.class,
                        "Can't find 'annees' while parsing page with nid: " + nType.getNid());
            ...
            if (calendrier == null) {
                DebugUtil.showError(MintageParser.class,
                        "Can't find Calendar while parsing page with nid: " + nType.getNid());
                return ParseEvent.ERROR;
            }

            Calendar calendar = N4JUtil.getInstance().numistaService.calendarService.findByCode(calendrier.get("value"),
                    calendrier.get("text"));
            ...
            for (Element trElement : trElements) {
                ...
                if (!tdElements.isEmpty() && !tdElements.get(0).children().isEmpty()) {
                    ...
                    if (variantNid.startsWith("nd")) {
                        ...
                        variant = nType.getVariants().stream().filter(v -> v.getNid().equals(varNid)).findFirst()
                                .orElse(null);
                        if (variant == null) {
                        ...
                        DebugUtil.showWarning(MintageParser.class,
                                "Find string with not 'nd' and 'nouveau' prefix while parsing page with nid: "
                                        + nType.getNid());
                        continue;
                    }
                } else {
                    ...
                    switch (tdElement.attr("class")) {
                        case "date_check" -> {
                            ...
                            if (dateCheckElement != null && dateCheckElement.attr("checked").equals("checked")) {
                                ...
                                if (datesElement == null) {
                                    DebugUtil.showWarning(MintageParser.class,
                                            "The date_check is checked but can't find Element with id=#dates" + name
                                                    + " while parsing page with nid: " + nType.getNid());
                                    continue;
                                }
                                ...
                                if (inputDatesElements.size() == 2) {
                                    ...
                                    if (StringUtils.isNumeric(dateFrom)) {
                                        Year yearFrom = N4JUtil.getInstance().numistaService.yearService
                                                .findYearByValueAndCalendar_Code(Integer.valueOf(dateFrom),
                                                        Calendar.GREGORIAN_CODE);
                                        variant.setYearFrom(yearFrom);
                                    }
                                    if (StringUtils.isNumeric(dateTill)) {
                                        Year yearTill = N4JUtil.getInstance().numistaService.yearService
                                                .findYearByValueAndCalendar_Code(Integer.valueOf(dateTill),
                                                        Calendar.GREGORIAN_CODE);
                        ...
                        case "date_year" -> {
                            ...
                            if (millesimeElement == null) {
                                DebugUtil.showWarning(MintageParser.class,
                                        "Can't find 'millesime' input while parsing page with nid: " + nType.getNid());
                                continue;
                            }
                            if (millesimeElement.attr("value").isEmpty()) {
                                DebugUtil.showInfo(MintageParser.class,
                                        "Value of 'millesime' is empty input while parsing page with nid: "
                                                + nType.getNid());
                                continue;
                            }
                            ...
                            if (millesimeElement.attr("name").startsWith("millesime")) {
                                ...
                                if (variant.getYear() != null && Objects.equals(variant.getYear().getValue(), yearValue)
                                        && variant.getYear().getCalendar().equals(calendar)) {
                                    continue;
                                }

                                Year year = N4JUtil.getInstance().numistaService.yearService
                                        .findYearByValueAndCalendar_Code(yearValue, calendar.getCode());
                                if (year == null) {
                                    year = N4JUtil.getInstance().numistaService.yearService
                                            .save(new Year(yearValue, calendar));
                                }
                        ...
                        case "date_mint" -> {
                            ...
                            if (atelierElement == null) {
                                DebugUtil.showWarning(MintageParser.class,
                                        "Can't find 'atelier' input while parsing page with nid: " + nType.getNid());
                                continue;
                            }
                        ...
                        case "date_mark" -> {
                            ...
                            if (!variant.getMarks().isEmpty()) {
                                ...
                                variant.getMarks()
                                        .removeIf(mark -> marks.stream().noneMatch(s -> mark.getNid().equals(s)));
                                if (variant.getMarks().size() != countMarks) {
                                    result = ParseEvent.CHANGED;
                                    DebugUtil.showInfo(MintageParser.class,
                                            "Find stale marks in variant with nid=" + variant.getNid());
                                }
                            }
                            ...
                            for (String markNid : marks) {
                                if (variant.getMarks().stream().noneMatch(s -> s.getNid().equals(markNid))) {
                                    ...
                                    DebugUtil.showInfo(MintageParser.class,
                                            "Add new mark for variant with nid=" + variant.getNid());
                                    result = ParseEvent.CHANGED;
                                }
                            }
                        }
                            ...
                            if (input != null) {
                                ...
                                if (mintage_str != null) {
                                    ...
                                    if (variant.getMintage() != mintage) {
                                        ...
                                        DebugUtil.showInfo(MintageParser.class,
                                                "Set new mintage for variant with nid=" + variant.getNid());
                                        result = ParseEvent.CHANGED;
                                    }
                                }
                            }
                        ...
                        case "" -> {
                            ...
                            if (variant.getCatalogueReferences() != null) {
                                ...
                                variant.getCatalogueReferences().removeIf(catalogueReference -> references.stream()
                                        .noneMatch(reference -> reference.getCatalogueNid()
                                                .equals(catalogueReference.getCatalogue().getNid())
                                                && reference.getNumber().equals(catalogueReference.getNumber())));

                                if (variant.getCatalogueReferences().size() != countReferences) {
                                    DebugUtil.showWarning(MintageParser.class,
                                            "Find stale references for variant with nid=" + variant.getNid());
                                    result = ParseEvent.CHANGED;
                                }
                            }

                            for (ReferenceToCatalogue reference : references) {
                                if (variant.getCatalogueReferences().stream().noneMatch(
                                        ref -> ref.getCatalogue().getNid().equals(reference.getCatalogueNid())
                                                && ref.getNumber().equals(reference.getNumber()))) {

                                    Catalogue catalogue = N4JUtil.getInstance().numistaService.catalogueService
                                            .findByNid(reference.catalogueNid, reference.catalogueCode);
                                    if (catalogue == null) {
                                        ReferenceNumberParser.parseReferenceCataloguesByCode(reference.catalogueCode);
                                        catalogue = N4JUtil.getInstance().numistaService.catalogueService
                                                .findByNid(reference.catalogueNid, reference.catalogueCode);

                                        if (catalogue == null) {
                                            DebugUtil.showError(MintageParser.class,
                                                    "Error with find or create new Catalogue with nid="
                                                            + reference.catalogueNid + " and code="
                                                            + reference.catalogueCode + ".");
                                            return ParseEvent.ERROR;
                                        }
                                    }

                                    CatalogueReference catalogueReference = N4JUtil
                                            .getInstance().numistaService.catalogueReferenceService
                                            .findByNumberAndCatalogueNid(reference.number, catalogue);
                                    if (catalogueReference == null) {
                                        DebugUtil.showError(MintageParser.class,
                                                "New CatalogueReference with nid=" + reference.catalogueNid
                                                        + " and code=" + reference.catalogueCode + " and number="
                                                        + reference.number + " can't be created.");
                                        return ParseEvent.ERROR;
                                    }

                                    variant.getCatalogueReferences().add(catalogueReference);
                                    DebugUtil.showInfo(MintageParser.class, "Add new CatalogueReference with number="
                                            + catalogueReference.getNumber() + " and catalogue code="
                                            + catalogueReference.getCatalogue().getCode() + " and catalogue nid="
                                            + catalogueReference.getCatalogue().getNid() + "");
                                    result = ParseEvent.CHANGED;
                                }
                        ...
                        case "yearline_comment" -> {
                            ...
                            if (comment != null) {
                                if (variant.getComment() != null && !variant.getComment().equals(comment)) {
                                    DebugUtil.showInfo(MintageParser.class,
                                            "Change comment to variant with nid=" + variant.getNid());
                                } else if (variant.getComment() == null) {
                                    DebugUtil.showInfo(MintageParser.class,
                                            "Add comment to variant with nid=" + variant.getNid());
