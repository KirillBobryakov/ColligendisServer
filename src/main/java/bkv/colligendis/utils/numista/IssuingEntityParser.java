...
public class IssuingEntityParser extends NumistaPartParser {
    ...
    public IssuingEntityParser() {
        super((page, nType) -> {
            ...
            if (issuingEntityCodeList.isEmpty()) {
                // DebugUtil.showWarning(RulerParser.class, "Can't find any EssuingEntity on
                // page for Ntype with nid : " + nType.getNid());
            ...
            nType.getIssuingEntities().removeIf(issuingEntity -> finalIssuingEntityCodeList.stream()
                    .noneMatch(issuingEntityCode -> issuingEntityCode.equals(issuingEntity.getCode())));

            if (nType.getIssuingEntities().size() != countIssuingEntities)
            ...
            for (String issuingEntityCode : issuingEntityCodeList) {
                ...
                if (nType.getIssuingEntities().stream()
                        .noneMatch(issuingEntity -> issuingEntity.getCode().equals(issuingEntityCode))) {
                    IssuingEntity issuingEntity = N4JUtil.getInstance().numistaService.issuingEntityService
                            .findIssuingEntityByCode(issuingEntityCode);
                    ...
                    if (issuingEntity == null || !issuingEntity.getIsActual()) {
                        if (!parseIssuingEntitiesByIssuerCodeFromPHPRequest(nType.getIssuer())) {
                            DebugUtil.showError(RulerParser.class,
                                    "Can't parse Rulers by PHP request with URL " + ISSUING_ENTITIES_BY_ISSUER_PREFIX
                    ...
                    issuingEntity = N4JUtil.getInstance().numistaService.issuingEntityService
                            .findIssuingEntityByCode(issuingEntityCode);
                    if (issuingEntity == null && issuingEntityCode.equals("1754")) {
                        DebugUtil.showError(IssuingEntityParser.class,
                                "Can't find IssuingEntity with code: " + issuingEntityCode + " for Ntype with nid : "
                                        + nType.getNid());
                    } else {
    ...
    private static boolean parseIssuingEntitiesByIssuerCodeFromPHPRequest(Issuer issuer) {
        Document issuingEntitiesPHPDocument = loadPageByURL(ISSUING_ENTITIES_BY_ISSUER_PREFIX + issuer.getCode(),
                false);

        if (issuingEntitiesPHPDocument == null) {
            DebugUtil.showError(IssuingEntityParser.class, "Can't load PHP IssuingEntities while parsing page.");
            return false;
        }
        ...
        if (!optgroups.isEmpty()) { // need to understand what to do with OPTGROUP in IssuingEntities
            DebugUtil.showError(
                    IssuingEntityParser.class,
                    "Find OPTGROUP while parsing IssuingEntities from PHP request with issuer's code: "
                            + issuer.getCode());
            return false;
        }

        List<IssuingEntity> issuingEntityList = N4JUtil.getInstance().numistaService.issuingEntityService
                .findIssuingEntitiesByIssuer(issuer);
        ...
        for (Element element : options) {
            ...
            IssuingEntity ie = N4JUtil.getInstance().numistaService.issuingEntityService
                    .findIssuingEntityByCode(ieCode);
            if (ie == null) {
