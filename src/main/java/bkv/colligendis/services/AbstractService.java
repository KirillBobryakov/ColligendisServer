...
public abstract class AbstractService<E extends AbstractEntity, R extends AbstractNeo4jRepository<E>> {
    ...
    public E save(E entity){
        ...
        if(isCashed){
            cash.stream().filter(e -> e.getUuid() == saved.getUuid()).findFirst().ifPresent(cashed -> cash.remove(cashed));
            cash.add(saved);
        }
        return saved;
    }
...
//        return repository.findEntityByUuid(uuid.toString());
     ...
     */
    public void detachEntityFromAnotherEntityWithRelationshipType(UUID firstEntityUuid, UUID secondEntityUuid, String relationshipType){
        repository.detachEntityFromAnotherEntityWithRelationshipType(firstEntityUuid.toString(), secondEntityUuid.toString(), relationshipType);
    }
